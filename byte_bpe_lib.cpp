...
void apply_env_overrides(Config &cfg, const std::unordered_map<std::string, std::string> &env) {
  ...
  if (auto v = get("DATA_PATH")) cfg.data_glob = *v;
  if (auto v = get("DATA_GLOB")) cfg.data_glob = *v;
  if (auto v = get("TEXT_FIELD")) cfg.text_field = *v;
  if (auto v = get("VOCAB_SIZE")) cfg.vocab_size = parse_size(*v, cfg.vocab_size);
  if (auto v = get("MIN_FREQ")) cfg.min_freq = parse_size(*v, cfg.min_freq);
  if (auto v = get("MIN_PAIR_FREQ")) cfg.min_pair_freq = parse_size(*v, cfg.min_pair_freq);
  if (auto v = get("CHUNK_FILES")) cfg.chunk_files = parse_size(*v, cfg.chunk_files);
  if (auto v = get("CHUNK_DOCS")) cfg.chunk_docs = parse_size(*v, cfg.chunk_docs);
  if (auto v = get("TOP_K")) cfg.top_k = parse_size(*v, cfg.top_k);
  if (auto v = get("MAX_CHARS_PER_DOC")) cfg.max_chars_per_doc = parse_size(*v, cfg.max_chars_per_doc);
  if (auto v = get("THREADS")) cfg.threads = parse_size(*v, cfg.threads);
  if (auto v = get("OUTPUT_JSON")) cfg.output_json = *v;
  if (auto v = get("OUTPUT_VOCAB")) cfg.output_vocab = *v;
  if (auto v = get("OUTPUT_MERGES")) cfg.output_merges = *v;
  if (auto v = get("UNK_TOKEN")) cfg.unk_token = *v;
  if (auto v = get("CHUNK_DIR")) cfg.chunk_dir = *v;
  if (auto v = get("RESUME")) cfg.resume = parse_bool(*v, cfg.resume);
  if (auto v = get("PROGRESS_INTERVAL")) cfg.progress_interval_ms = parse_size(*v, cfg.progress_interval_ms);
  if (auto v = get("PROGRESS_INTERVAL_MS")) cfg.progress_interval_ms = parse_size(*v, cfg.progress_interval_ms);
...
static bool is_space(uint32_t cp) {
  ...
  return cp == 0x85 || cp == 0xA0 || cp == 0x1680 || cp == 0x180E ||
...
std::string byte_level_encode(const std::string &token,
...
std::unordered_map<std::string, uint32_t> reduce_top_k(
    std::unordered_map<std::string, uint32_t> &local, std::size_t top_k) {
...
bool write_tokenizer_json(const std::string &path,
                          const Config &cfg,
...
bool write_chunk_file(const std::string &path,
...
bool merge_chunk_file(const std::string &path,
...
void ProgressTracker::maybe_print(bool force) {
  ...
  double eta = (chunk_rate > 0.0 && total_ > done_chunks)
                   ? static_cast<double>(total_ - done_chunks) / chunk_rate
                   : 0.0;

  std::ostringstream oss;
  oss.setf(std::ios::fixed);
  oss << "[" << label_ << "] chunks " << done_chunks << "/" << total_ << " ("
      << std::setprecision(1) << pct << "%)";
  if (done_docs > 0) {
    oss << " docs " << done_docs << " (" << std::setprecision(1)
        << (static_cast<double>(done_docs) / 1e6) << "M)";
  }
  if (chunk_rate > 0.0) {
